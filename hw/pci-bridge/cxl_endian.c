...
void correct_compl_hdr(cxl_io_completion_header_t *compl_hdr) 
{
    ...
    recv_hdr.byte_count = (compl_hdr->byte_count_upper << 8) | compl_hdr->byte_count_lower;
...
void correct_payload_io(uint8_t *payload_bytes, cxl_io_fmt_type_t pld_fmt)
{
    ...
    switch (pld_fmt) 
    {
        ...
        case MWR_64B: 
        {
            ...
            correct_mreq_hdr((cxl_io_mreq_header_t *) 
        ...
        case CFG_WR1:
        {
            correct_cfgq_hdr((cxl_io_cfg_req_header_t *)
                (payload_bytes + sizeof(cxl_io_header_t)));
            break;
        }
        ...
        case CPL:
        {
            correct_compl_hdr((cxl_io_completion_header_t *)
                (payload_bytes + sizeof(cxl_io_header_t)));
            break;
        }
        ...
        case CPL_D:
        {
            correct_compl_hdr((cxl_io_completion_header_t *)
                (payload_bytes + sizeof(cxl_io_header_t)));
            /* perform_endian_swap((uint8_t *) 
                (payload_bytes + sizeof(cxl_io_completion_packet_t)),
