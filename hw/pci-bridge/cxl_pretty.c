...
void snpprintpacket(char *buf, void *pckt, size_t at_most)
{
    ...
    enum CXL_PAYLOAD_TYPE pload_type = ((system_header_packet_t *) pckt_data)->payload_type;
    if (pload_type != CXL_IO) return; // ignore a non-CXL.io packet

    cxl_io_fmt_type_t io_fmt = ((cxl_io_header_t *) 
        (pckt_data + sizeof(system_header_packet_t)))->fmt_type;

    void *pload_ptr = (void *) (pckt_data + sizeof(system_header_packet_t) + sizeof(cxl_io_header_t));
    ...
    switch (io_fmt)
    {
        ...
        case CPL:
        {
            cxl_io_completion_header_t pload = * (cxl_io_completion_header_t *) pload_ptr;
            snprintf(
                buf, 
                at_most, 
                "[CPL PACKET] \n"
        ...
        {
            cxl_io_completion_header_t pload = * (cxl_io_completion_header_t *) pload_ptr;
            uint64_t data = * (uint64_t *) (pload_ptr + sizeof(cxl_io_completion_header_t));
            snprintf(
                buf, 
                at_most, 
                "[CPL PACKET + DATA] \n"
